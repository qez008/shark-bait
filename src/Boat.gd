...
@export var surf_boost = 5.0
@export var steering_rate = 12.0
@export var displacement_rate = 0.8
@export var max_buoyancy = 70.0
@export var slamming_force = 0.7
@export var linear_water_resistance = 4.0
@export var angular_water_resistance = 10.0
@export var heel_curve: Curve

@export var bow: Node3D
@export var stern: Node3D
...
func _physics_process(_delta):
	...
	_heel = Quaternion(transform.basis).x
...
func apply_keel_torque(transform_up: Vector3, transform_sideways: Vector3):
	...
	add_constant_torque(global_transform.basis * keel_force)
...
func apply_sail_torque(transform_up: Vector3, wind: Wind):
	...
	add_constant_torque(sail_force)


func move_in_water():
	var direction = global_transform.basis * Vector3.RIGHT
	var slope = wave_slope()

	# accelerate
	if Input.is_action_pressed("ui_up"):
		add_constant_central_force(direction * acceleration)
		# apply speed boost when going 'downhill' i.e. surfing
		if slope > 0.0:
			var v = Vector3.RIGHT * surf_boost * sqrt(slope * 2.5)
			add_constant_central_force(global_transform.basis * v)
	...
	if input_direction != 0:
		...
		add_constant_torque(global_transform.basis * rot)
...
func apply_buoyancy():
	...
	for i in num_floaters:
		...
		var relative_position = floater_position - global_transform.origin
		...
		if depth > 0:
			...
			if not _floaters_in_water[i]:
				...
				apply_impulse(relative_position, impact_force / num_floaters)
		...
		add_constant_force(force, relative_position)
...
func is_in_water() -> bool:
	...
	for floater in floaters.get_children():
		var floater_position = (floater as Node3D).global_transform.origin
...
func relative_wind_angle() -> float:
	...
	var relative_wind_vector = global_transform.basis * wind_vector
