...
func _ready():
	_shader_mat = self.mesh.surface_get_material(0)

	var _s1 = WaveManager.connect("update_wave_config", set_waves)
	var _s2 = WaveManager.connect("update_wave_offset", set_wave_offset)
